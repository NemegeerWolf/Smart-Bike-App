...
namespace SmartBike_Api.Functions.Gets
{
    public static class GetAllScoresGame
    {
        ...
        public static async Task<IActionResult> GetGameScoreAll(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "smartbike/game/{gameid}")] HttpRequest req, int gameid,
            ILogger log)
        {
            ...
            {
                QueryDefinition query = new QueryDefinition("select * from Games i where i.user!=null AND i.gameId = @gameId  order by i.speed desc").WithParameter("@gameId", gameid);
                List<Game> items = await GetScoresAsync(query);
                return new OkObjectResult(AddRank(items));
            }
            else if (gameid == 3)
